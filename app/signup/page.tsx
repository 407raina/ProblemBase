...
export default function SignUpPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!data.user) {
        setErrorMsg("Sign up did not complete. Please try again.");
        setLoading(false);
        return;
      }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4 py-8">
      <section className="w-full max-w-md rounded-2xl border border-neutral-200 bg-white/95 p-6 shadow-sm backdrop-blur">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight text-neutral-950">Sign up</h1>
        <p className="mt-2 text-sm text-neutral-600">Create an account to post problems and track builds.</p>
        ...
        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 outline-none transition focus:border-neutral-500"
            />
            <p className="mt-1 text-xs text-neutral-500">Use at least 6 characters.</p>
          </div>
        ...
        <p className="mt-4 text-sm text-neutral-600">
          ...
          <Link href="/signin" className="font-medium text-neutral-950 underline underline-offset-4">
            Sign in
          </Link>
        </p>
      </section>
